...
class DirectAssessmentWindowDocumentTask(BaseMetadata):
    """
    ...
    def completed_items_for_user(self, user):
        results = DirectAssessmentWindowDocumentResult.objects.filter(
    ...
    def next_item_for_user(self, user, return_completed_items=False):
        ...
        for item in self.items.all().order_by('id'):
            result = DirectAssessmentWindowDocumentResult.objects.filter(
        ...
        if not next_item:
            ...
            annotations = DirectAssessmentWindowDocumentResult.objects.filter(
    ...
    def next_document_for_user(self, user, return_statistics=True):
        ...
        completed_blocks = DirectAssessmentWindowDocumentResult.objects.filter(
            task=self,
    ...
    def get_results_for_each_item(self, block_items, user):
        ...
        for item in block_items:
            result = (
                DirectAssessmentWindowDocumentResult.objects.filter(
                    item__id=item.id,
                    completed=True,
    ...
    def import_from_json(cls, campaign, batch_user, batch_data, max_count):
        """
        Creates new DirectAssessmentWindowDocumentTask instances based on JSON input.
        """
        ...
        for batch_task in batch_json:
            ...
            new_task = DirectAssessmentWindowDocumentTask(
                campaign=campaign,
...
class DirectAssessmentWindowDocumentResult(BaseMetadata):
    """
