...
from EvalData.models import DirectAssessmentWindowDocumentResult
from EvalData.models import DirectAssessmentWindowDocumentTask
...
def direct_assessment_document(request, code=None, campaign_name=None):
    ...
    return render(request, 'EvalView/direct-assessment-document-setup2.html', context)
...
# pylint: disable=C0103,C0330
@login_required
def direct_assessment_window_document (request, code=None, campaign_name=None):
    """
    Direct assessment document annotation view.
    """
    t1 = datetime.now()
    ...
    # If language code has been given, find a free task and assign to user.
    if not current_task:
        current_task = DirectAssessmentWindowDocumentTask.get_task_for_user(user=request.user)
    ...
    if not current_task:
        ...
        next_task = DirectAssessmentWindowDocumentTask.get_next_free_task_for_language(
            code, campaign, request.user
        )
        ...
        if score and item_id and start_timestamp and end_timestamp:
            ...
            if current_item.documentID == document_id:
                ...
                if current_item.itemID == int(item_id) and current_item.id == int(
                    ...
                    utc_now = datetime.utcnow().replace(tzinfo=utc)
                    # pylint: disable=E1101
                    DirectAssessmentWindowDocumentResult.objects.create(
                        score=score,
                        start_time=float(start_timestamp),
                        end_time=float(end_timestamp),
                        item=current_item,
    ...
    # requests
    context = {
        'active_page': 'direct-assessment-window-document',
        'item_id': current_item.itemID,
        'task_id': current_item.id,
        'document_id': current_item.documentID,
        'completed_blocks': completed_blocks,
    ...
    return render(request, 'EvalView/direct-assessment-window-document.html', context)